<?php
	...
	*/
	...
	$row_sql = mysql_query( "SELECT `generation_number`, `population_genes` FROM `SIMPL_GENOMES` WHERE `population_size` = $population_size AND `number_of_genes_per_genome` = $number_of_genes_per_genome ORDER BY `generation_number` DESC LIMIT 1" );
	$row = mysql_fetch_array( $row_sql );
	echo $row[ 'generation_number' ] . ";" . $row[ 'population_genes' ];
